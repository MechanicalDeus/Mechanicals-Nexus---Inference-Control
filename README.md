...
**Case study (two repos, zero file opens):** **[`docs/case-study-cross-repo-orientation.md`](docs/case-study-cross-repo-orientation.md)** — cross-checkout comparison via **`nexus.cli_opc`** only; **~110k** session tokens vs **naive full-`.py` ingest** at **TTRPG-scale** (**~65 MB** `.py` in one measured tree — **~7.1 GB** total disk; **Aether ~605 MB** total / **~507 KB** `.py` — see **Measured checkout sizes** in that doc).
