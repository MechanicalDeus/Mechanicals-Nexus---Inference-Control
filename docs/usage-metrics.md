...
| `cursor-cross-repo-orientation-110k.png` | **Cross-repo case study** (two local checkouts, **no source files opened** in the agent — Nexus `map`/`locate`/`grep` only): session total **~110k** tokens with **Cache Read** dominant. Narrative: **[`case-study-cross-repo-orientation.md`](case-study-cross-repo-orientation.md)**. |
